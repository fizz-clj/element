...
(deftest test-create-proxy
  (let [original-target
        ...
        name-before-update (.getName ^js/HTMLElement proxy)

        proto-before-update (.-prototype proxy)

        _ (reset! !handler updated-target)

        name-after-update (.getName ^js/HTMLElement proxy)
...
(deftest test-proxy-patches-new-observed-attributes
  (testing "Given that a new observed attribute is added
    ...
    ;; this is the wrong solution to this problem... instead, 
    ;; we need to intercept the setAttribute and removeAttribute call
    (let [el-name "my-custom-el"
          ...
          original-target ;; this is a proto, needs to be a ctor
          (js/Object.create
           js/HTMLElement
